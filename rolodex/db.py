...
def _haystack(s: dict, notes: list[str]) -> str:
    ...
    parts = [s["company"], s["contact_name"], s["address"], s["website"], s["summary"],
             " ".join(s["categories"]), json.dumps(p.get("products", [])), json.dumps(p.get("locations", [])),
             json.dumps(p.get("certifications", [])), " ".join(notes)]
    return " ".join(parts).lower()


def search(q: str = "", category: str = "", attention: bool = False) -> list[dict]:
    """Plain keyword search: every word must appear somewhere in the profile or notes."""
